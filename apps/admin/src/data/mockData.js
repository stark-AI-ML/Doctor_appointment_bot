/**
 * Mock Data for development.
 * 
 * This file provides realistic dummy data so the UI works
 * without a backend. When VITE_USE_MOCK=false, this file is never used.
 * 
 * Structure mirrors the expected API response shapes exactly,
 * making the switch to real data seamless.
 */

// ── Doctors ──
export const mockDoctors = [
  {
    id: 1,
    name: 'Dr. Sharma',
    specialization: 'General Physician',
    consultation_fee: 500,
    is_active: true,
    avatar: null,
    phone: '+91 98765 43210',
    email: 'sharma@clinic.com',
    created_at: '2026-07-15T10:00:00Z',
  },
  {
    id: 2,
    name: 'Dr. Verma',
    specialization: 'Dermatologist',
    consultation_fee: 800,
    is_active: true,
    avatar: null,
    phone: '+91 98765 43211',
    email: 'verma@clinic.com',
    created_at: '2026-07-20T10:00:00Z',
  },
  {
    id: 3,
    name: 'Dr. Gupta',
    specialization: 'Orthopedic',
    consultation_fee: 1000,
    is_active: true,
    avatar: null,
    phone: '+91 98765 43212',
    email: 'gupta@clinic.com',
    created_at: '2026-08-01T10:00:00Z',
  },
  {
    id: 4,
    name: 'Dr. Patel',
    specialization: 'Cardiologist',
    consultation_fee: 1200,
    is_active: false,
    avatar: null,
    phone: '+91 98765 43213',
    email: 'patel@clinic.com',
    created_at: '2026-08-10T10:00:00Z',
  },
]

// ── Services ──
export const mockServices = [
  { id: 1, name: 'General Consultation', description: 'Regular checkup', is_active: true },
  { id: 2, name: 'Skin Treatment', description: 'Dermatology services', is_active: true },
  { id: 3, name: 'Bone & Joint', description: 'Orthopedic consultation', is_active: true },
  { id: 4, name: 'Heart Checkup', description: 'Cardiac screening', is_active: true },
]

// ── Bookings ──
export const mockBookings = [
  {
    id: 1,
    booking_id: 'BK-20260820-001',
    doctor_id: 2,
    doctor_name: 'Dr. Verma',
    service_id: 1,
    service_name: 'General Consultation',
    patient_name: 'Rahul Kumar',
    mobile: '9876543210',
    date: '2026-08-20',
    time_slot: '11:00 AM',
    status: 'confirmed',
    created_at: '2026-08-19T14:30:00Z',
    updated_at: '2026-08-19T15:00:00Z',
  },
  {
    id: 2,
    booking_id: 'BK-20260820-002',
    doctor_id: 1,
    doctor_name: 'Dr. Sharma',
    service_id: 1,
    service_name: 'General Consultation',
    patient_name: 'Neha Singh',
    mobile: '9876543211',
    date: '2026-08-20',
    time_slot: '10:00 AM',
    status: 'confirmed',
    created_at: '2026-08-19T16:00:00Z',
    updated_at: '2026-08-19T16:30:00Z',
  },
  {
    id: 3,
    booking_id: 'BK-20260820-003',
    doctor_id: 3,
    doctor_name: 'Dr. Gupta',
    service_id: 3,
    service_name: 'Bone & Joint',
    patient_name: 'Amit Gupta',
    mobile: '9876543212',
    date: '2026-08-20',
    time_slot: '12:00 PM',
    status: 'pending',
    created_at: '2026-08-20T08:00:00Z',
    updated_at: '2026-08-20T08:00:00Z',
  },
  {
    id: 4,
    booking_id: 'BK-20260820-004',
    doctor_id: 2,
    doctor_name: 'Dr. Verma',
    service_id: 2,
    service_name: 'Skin Treatment',
    patient_name: 'Pooja Yadav',
    mobile: '9876543213',
    date: '2026-08-21',
    time_slot: '05:00 PM',
    status: 'completed',
    created_at: '2026-08-20T09:00:00Z',
    updated_at: '2026-08-21T17:30:00Z',
  },
  {
    id: 5,
    booking_id: 'BK-20260822-001',
    doctor_id: 1,
    doctor_name: 'Dr. Sharma',
    service_id: 1,
    service_name: 'General Consultation',
    patient_name: 'Vikram Mehra',
    mobile: '9876543214',
    date: '2026-08-22',
    time_slot: '09:00 AM',
    status: 'cancelled',
    created_at: '2026-08-21T11:00:00Z',
    updated_at: '2026-08-22T08:00:00Z',
  },
  {
    id: 6,
    booking_id: 'BK-20260825-001',
    doctor_id: 3,
    doctor_name: 'Dr. Gupta',
    service_id: 3,
    service_name: 'Bone & Joint',
    patient_name: 'Sneha Kapoor',
    mobile: '9876543215',
    date: '2026-08-25',
    time_slot: '02:00 PM',
    status: 'confirmed',
    created_at: '2026-08-24T10:00:00Z',
    updated_at: '2026-08-24T10:30:00Z',
  },
  {
    id: 7,
    booking_id: 'BK-20260826-001',
    doctor_id: 2,
    doctor_name: 'Dr. Verma',
    service_id: 2,
    service_name: 'Skin Treatment',
    patient_name: 'Ravi Sharma',
    mobile: '9876543216',
    date: '2026-08-26',
    time_slot: '11:30 AM',
    status: 'pending',
    created_at: '2026-08-25T16:00:00Z',
    updated_at: '2026-08-25T16:00:00Z',
  },
  {
    id: 8,
    booking_id: 'BK-20260828-001',
    doctor_id: 1,
    doctor_name: 'Dr. Sharma',
    service_id: 1,
    service_name: 'General Consultation',
    patient_name: 'Priya Nair',
    mobile: '9876543217',
    date: '2026-08-28',
    time_slot: '04:00 PM',
    status: 'confirmed',
    created_at: '2026-08-27T09:00:00Z',
    updated_at: '2026-08-27T09:30:00Z',
  },
  {
    id: 9,
    booking_id: 'BK-20260830-001',
    doctor_id: 3,
    doctor_name: 'Dr. Gupta',
    service_id: 3,
    service_name: 'Bone & Joint',
    patient_name: 'Arjun Reddy',
    mobile: '9876543218',
    date: '2026-08-30',
    time_slot: '10:30 AM',
    status: 'completed',
    created_at: '2026-08-29T14:00:00Z',
    updated_at: '2026-08-30T11:00:00Z',
  },
  {
    id: 10,
    booking_id: 'BK-20260901-001',
    doctor_id: 2,
    doctor_name: 'Dr. Verma',
    service_id: 2,
    service_name: 'Skin Treatment',
    patient_name: 'Kavita Joshi',
    mobile: '9876543219',
    date: '2026-09-01',
    time_slot: '03:00 PM',
    status: 'pending',
    created_at: '2026-08-31T12:00:00Z',
    updated_at: '2026-08-31T12:00:00Z',
  },
]

// ── Time Slots ──
export const mockTimeSlots = [
  { id: 1, doctor_id: 1, time: '09:00 AM', is_booked: false },
  { id: 2, doctor_id: 1, time: '10:00 AM', is_booked: true },
  { id: 3, doctor_id: 1, time: '11:00 AM', is_booked: false },
  { id: 4, doctor_id: 1, time: '12:00 PM', is_booked: false },
  { id: 5, doctor_id: 1, time: '02:00 PM', is_booked: true },
  { id: 6, doctor_id: 1, time: '03:00 PM', is_booked: false },
  { id: 7, doctor_id: 1, time: '04:00 PM', is_booked: true },
  { id: 8, doctor_id: 1, time: '05:00 PM', is_booked: false },
  { id: 9, doctor_id: 2, time: '10:00 AM', is_booked: true },
  { id: 10, doctor_id: 2, time: '11:00 AM', is_booked: true },
  { id: 11, doctor_id: 2, time: '11:30 AM', is_booked: true },
  { id: 12, doctor_id: 2, time: '12:00 PM', is_booked: false },
  { id: 13, doctor_id: 2, time: '03:00 PM', is_booked: true },
  { id: 14, doctor_id: 2, time: '04:00 PM', is_booked: false },
  { id: 15, doctor_id: 2, time: '05:00 PM', is_booked: true },
  { id: 16, doctor_id: 3, time: '09:00 AM', is_booked: false },
  { id: 17, doctor_id: 3, time: '10:00 AM', is_booked: false },
  { id: 18, doctor_id: 3, time: '10:30 AM', is_booked: true },
  { id: 19, doctor_id: 3, time: '12:00 PM', is_booked: true },
  { id: 20, doctor_id: 3, time: '02:00 PM', is_booked: true },
  { id: 21, doctor_id: 3, time: '03:00 PM', is_booked: false },
  { id: 22, doctor_id: 3, time: '04:00 PM', is_booked: false },
]

// ── Dashboard Stats ──
export const mockDashboardStats = {
  totalBookings: 128,
  todayBookings: 24,
  confirmed: 98,
  cancelled: 10,
  totalRevenue: 86400,
  totalDoctors: 4,
  activeDoctors: 3,
  totalPatients: 95,
}

// ── Chart Data (Last 7 days) ──
export const mockChartData = [
  { date: 'Aug 26', bookings: 12, confirmed: 9, cancelled: 1 },
  { date: 'Aug 27', bookings: 18, confirmed: 15, cancelled: 2 },
  { date: 'Aug 28', bookings: 15, confirmed: 12, cancelled: 1 },
  { date: 'Aug 29', bookings: 22, confirmed: 18, cancelled: 3 },
  { date: 'Aug 30', bookings: 19, confirmed: 16, cancelled: 1 },
  { date: 'Aug 31', bookings: 25, confirmed: 20, cancelled: 2 },
  { date: 'Sep 01', bookings: 17, confirmed: 8, cancelled: 0 },
]

// ── Patients (derived from bookings) ──
export const mockPatients = [
  { id: 1, name: 'Rahul Kumar', mobile: '9876543210', total_bookings: 5, last_visit: '2026-08-20' },
  { id: 2, name: 'Neha Singh', mobile: '9876543211', total_bookings: 3, last_visit: '2026-08-20' },
  { id: 3, name: 'Amit Gupta', mobile: '9876543212', total_bookings: 2, last_visit: '2026-08-20' },
  { id: 4, name: 'Pooja Yadav', mobile: '9876543213', total_bookings: 4, last_visit: '2026-08-21' },
  { id: 5, name: 'Vikram Mehra', mobile: '9876543214', total_bookings: 1, last_visit: '2026-08-22' },
  { id: 6, name: 'Sneha Kapoor', mobile: '9876543215', total_bookings: 6, last_visit: '2026-08-25' },
  { id: 7, name: 'Ravi Sharma', mobile: '9876543216', total_bookings: 2, last_visit: '2026-08-26' },
  { id: 8, name: 'Priya Nair', mobile: '9876543217', total_bookings: 3, last_visit: '2026-08-28' },
  { id: 9, name: 'Arjun Reddy', mobile: '9876543218', total_bookings: 7, last_visit: '2026-08-30' },
  { id: 10, name: 'Kavita Joshi', mobile: '9876543219', total_bookings: 1, last_visit: '2026-09-01' },
]

// ── Mock User (Admin) ──
export const mockUser = {
  id: 1,
  name: 'Admin',
  email: 'admin@docbot.com',
  role: 'admin',
}

// ── Settings ──
export const mockSettings = {
  clinic_name: 'DocBot Clinic',
  clinic_address: '123 Health Street, Medical City',
  clinic_phone: '+91 98765 00000',
  whatsapp_number: '9161138859',
  whatsapp_api_status: 'connected',
  notification_booking_confirm: true,
  notification_booking_reminder: true,
  notification_booking_cancel: true,
  reminder_hours_before: 2,
}

// ── Doctor-wise Report ──
export const mockDoctorReport = [
  { doctor: 'Dr. Sharma', bookings: 42, revenue: 21000, completion_rate: 85 },
  { doctor: 'Dr. Verma', bookings: 38, revenue: 30400, completion_rate: 92 },
  { doctor: 'Dr. Gupta', bookings: 35, revenue: 35000, completion_rate: 78 },
  { doctor: 'Dr. Patel', bookings: 13, revenue: 15600, completion_rate: 88 },
]

// ── Status Distribution ──
export const mockStatusDistribution = [
  { name: 'Confirmed', value: 98, color: '#25D366' },
  { name: 'Pending', value: 15, color: '#f0ad4e' },
  { name: 'Completed', value: 5, color: '#58a6ff' },
  { name: 'Cancelled', value: 10, color: '#f85149' },
]
